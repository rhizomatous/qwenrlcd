#!/usr/bin/env bash
# Read-only Choice and Score diagnostics for the completed full core run.
set -euo pipefail
...
uv run pytest -q \
  tests/test_diagnose_choice.py \
  tests/test_diagnose_score.py \
  tests/test_choice_set_probe.py
uv run qwenrlcd-diagnose-choice --run-dir "$QWENRLCD_RUN_DIR"
